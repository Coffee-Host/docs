...
export function CustomViewOptionsPopover({
  markdownUrl,
  githubUrl,
  className,
  children,
  ...
  const items = useMemo(() => {
    ...
    return [
      ...
      markdownUrl && {
        title: '檢視 Markdown 原始碼',
        href: markdownUrl,
        icon: <TextIcon />,
      },
      {
        title: '在 Scira AI 中開啟',
  ...
  return (
    <Popover>
      <PopoverTrigger
        {...props}
        className={`${buttonVariants({ color: 'secondary', size: 'sm' })} gap-2 data-[state=open]:bg-fd-accent data-[state=open]:text-fd-accent-foreground ${className || ''}`}
      >
        {children ?? '開啟'}
